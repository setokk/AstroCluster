...
class ClusterServiceServicer(cluster_pb2_grpc.ClusterServiceServicer):
    ...
    def PerformClustering(self, request, context):
        ...
        project_path = f"/ac-clustering-service/projects/{request.path}"
        files, file_paths = project_traverser.get_project_files(traverser_strategy, f"{project_path}", request.extensions)
